...
const ArticleSchema = z.object({
  ...
  url: z.string(),
  summary: z.string(),
  inLlmContext: z.boolean().optional(),
});
...
function PartyCard({ party }: { party: Party }) {
  ...
  return (
    <div className="rounded-[10px] border border-[#dce6f1] bg-white p-3">
      <div className="flex items-start justify-between gap-4">
        <div className="flex w-full items-start gap-3">
          <div>
            <h3 className="m-0 text-[15px] font-bold text-[#1d3247]">{party.partyName}</h3>
            <div className="mt-1 flex flex-wrap items-center gap-3">
              <span className="inline-flex items-center rounded-full bg-[#eef6ff] px-2.5 py-[3px] text-xs font-semibold text-[#0b63a8]">
                Role: {party.role}
              </span>
              <span className="m-0 text-xs text-[#6b7280]">Articles Found: {party.articlesFound}</span>
              {hasAdverse && party.articles.length > 0 && (
                <button
                  ...
                  title="Show articles"
                  className="ml-auto flex h-8 w-8 items-center justify-center rounded-full border border-[#b8d3e8] bg-white text-[#0079c1] transition-colors hover:bg-[#eef6ff] hover:border-[#0079c1]"
                >
        ...
        <div className="ml-auto flex min-w-[210px] flex-col items-end gap-2">
          <div
            className={`inline-flex w-full max-w-[240px] items-center justify-center rounded-lg px-2.5 py-1.5 text-center text-xs font-semibold ${
              hasAdverse
      ...
      {hasAdverse && open && (
        <div className="mt-3 rounded-lg border-2 border-dashed border-[#2f76d2] bg-[#e9eff8] p-3.5">
          ...
          {party.articles.map((article) => (
            <div key={article.indexInResults} className="mb-3 rounded-[10px] border border-[#e2e8f0] bg-[#f8fafc] p-[14px_14px_12px] last:mb-0">
              <div className="mb-2.5 flex flex-wrap items-center gap-2.5">
                ...
                {article.inLlmContext && (
                  <span className="inline-flex items-center rounded-full border border-[#b7e4c7] bg-[#eaf9ef] px-2 py-0.5 text-[11px] font-bold tracking-[0.02em] text-[#147a32]">
                    In LLM Context
                  </span>
                )}
              </div>
...
export function AmSearchResults({ caseName, caseSummary, parties }: AmSearchResultsProps) {
  return (
    <div className="w-full max-w-2xl rounded-xl border border-[#d9e2ec] bg-white shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
      <div className="border-b border-[#d9e2ec] bg-gradient-to-b from-[#fafdff] to-[#f1f8fd] px-3.5 py-3">
        <h3 className="m-0 text-base font-bold text-[#0079c1]">Adverse Media Search — {caseName}</h3>
        <p className="m-0 mt-1.5 text-[13px] leading-[1.4] text-[#5f7182]">{caseSummary}</p>
      </div>
